...
def creating_fleet(board):
    ...
    print " Tutaj widzimy wylsowane stateczki"
    board_ships = print_board(board)
    while len(ships['four']) < 1:
        ships ['four'][randint(1, len(board))] = randint(1, len(board[0]))
        a = int(ships['four'].keys())
        b = int(ships['four'][len(ships['four'])])    # or ships['four'].values()
        board[a + (x in range (-1, 2))][b + (y in range(-1, 2))] == "X"  # create something that does not give a list
    return ships, board_ships


print creating_fleet(board)
